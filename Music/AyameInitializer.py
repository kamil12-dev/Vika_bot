...
class AyameInitializer:
    ...
    def getBot(self) -> AyameBot:
        return self.__bot

    def __create_bot(self, willListen: bool) -> AyameBot:
        if willListen:
            prefix = self.__config.BOT_PREFIX
            bot = AyameBot(listingSlash=True,
            ...
            bot = AyameBot(listingSlash=False,
                            command_prefix=prefix,
                            pm_help=True,
                            case_insensitive=True,
                            intents=self.__intents)
    ...
    def __add_cogs(self, bot: Bot) -> None:
        try:
            ...
            if len(bot.cogs.keys()) != self.__getTotalCogs():
                print(cogsStatus)
                raise AyameError(message='Failed to load some Cog')

        except AyameError as e:
            print(f'[Error Loading Ayame] -> {e.message}')
