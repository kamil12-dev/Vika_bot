...
class PlayerProcess(Process):
        ...
        while True:
            ...
            try:
                ...
                if type == VCommandsType.PAUSE:
                    ...
                    print(f'[ОШИБКА] -> Получена неизвестная команда: {command}')
            except Exception as e:
                print(f'[ОШИБКА В ПРИЕМНИКЕ КОМАНД] -> {type} - {e}')
            finally:
                self.__playerLock.release()
    ...
    async def __createBotInstance(self) -> AyameBot:
        """Load a new bot instance that should not be directly called."""
        initializer = AyameInitializer(willListen=False)
    ...
    async def __ensureDiscordConnection(self, bot: AyameBot) -> None:
