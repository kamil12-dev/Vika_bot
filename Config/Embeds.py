...
class VEmbeds:
    ...
    def __addFooterContent(self, embed: Embed) -> Embed:
        footerText = f'\u200b Пожалуйста, поддержите этот проект, оставив звездочку: {self.__config.PROJECT_URL}'
    ...
    def CUSTOM_ERROR(self, error: AyameError) -> Embed:
        embed = Embed(
            title=error.title,
    ...
    def RANDOM_NUMBER(self, a: int, b: int, x: int) -> Embed:
        embed = Embed(
            title=f'Случайное число между [{a}, {b}]',
            description=x,
